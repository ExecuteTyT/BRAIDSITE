...
const platforms: Record<string, PlatformConfig> = {
  ...
  windows: {
    ...
    faq: [
      ...
      { q: 'Подходит ли для игр?', a: 'Да! BRAID VPN оптимизирован для онлайн-игр с низким пингом. Серверы в России (~22ms) и Финляндии (~40ms).' },
    ],
  },
  mac: {
    slug: 'mac',
...
export const PlatformPage: React.FC<{ platform: string }> = ({ platform }) => {
  ...
  React.useEffect(() => {
    ...
    updateMeta({
      ...
      path: `/${config.slug}`,
      keywords: `vpn для ${config.title.toLowerCase()}, скачать vpn ${config.title.toLowerCase()}, braid vpn ${config.title.toLowerCase()}`,
    });
  }, [config]);
