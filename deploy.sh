...
SERVER_PATH="${DEPLOY_PATH:-/var/www/braidvpn.com}"
...
if [ "$SERVER_USER" = "your-username" ] || [ "$SERVER_IP" = "your-server-ip" ]; then
    ...
    echo -e "${YELLOW}   export DEPLOY_PATH=/var/www/braidvpn.com${NC}"
    echo ""
...
HTTP_CODE=$(curl -s -o /dev/null -w "%{http_code}" https://braidvpn.com)
...
echo -e "${GREEN}📋 Sitemap: https://braidvpn.com/sitemap.xml${NC}"
echo -e "${GREEN}🤖 Robots: https://braidvpn.com/robots.txt${NC}"
echo ""
