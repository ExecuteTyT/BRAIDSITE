...
if [ -f "/etc/letsencrypt/live/braidvpn.com/fullchain.pem" ]; then
    echo "✅ SSL сертификат найден"
    
    # Проверить, включает ли сертификат www
    CERT_DOMAINS=$(openssl x509 -in /etc/letsencrypt/live/braidvpn.com/fullchain.pem -text -noout | grep -oP 'DNS:\K[^\s,]+' || echo "")
    if [[ "$CERT_DOMAINS" == *"www.braidvpn.com"* ]]; then
        echo "✅ Сертификат включает www.braidvpn.com"
    else
        echo "⚠️  Сертификат не включает www.braidvpn.com"
        echo "   Рекомендуется обновить: sudo certbot certonly --nginx --expand -d braidvpn.com -d www.braidvpn.com"
    fi
else
    echo "⚠️  SSL сертификат не найден"
    echo "   Установите через: sudo certbot certonly --nginx -d braidvpn.com -d www.braidvpn.com"
fi
...
if [ -f "nginx-braidvpn.conf" ]; then
    ...
    # Исправить HTTP редирект (убрать $server_name, использовать braidvpn.com)
    sudo sed -i 's/return 301 https:\/\/\$server_name\$request_uri;/return 301 https:\/\/braidvpn.com$request_uri;/' "$CONFIG_FILE"
    
    # Заменить return 404 на правильный редирект
    sudo sed -i 's/return 404; # managed by Certbot/return 301 https:\/\/braidvpn.com$request_uri;/' "$CONFIG_FILE"
    
    # Добавить отдельный server блок для www (если его нет)
    if ! grep -q "server_name www.braidvpn.com" "$CONFIG_FILE"; then
...
if [ $? -eq 0 ]; then
    ...
    curl -I https://www.braidvpn.com 2>&1 | grep -i "location\|http" | head -2 || echo "⚠️  SSL сертификат для www может отсутствовать"
    echo ""
    echo "✅ Настройка завершена!"
    echo ""
    echo "📋 Следующие шаги:"
    echo "   1. Убедитесь, что SSL сертификат включает www: sudo certbot certonly --nginx --expand -d braidvpn.com -d www.braidvpn.com"
    echo "   2. В Yandex Webmaster укажите главное зеркало: https://braidvpn.com"
